...
LinearAllocator::LinearAllocator(Allocator& allocator, size_t capacity) :
    m_parentAllocator(&allocator), m_memoryBuffer(allocator.Alloc(capacity)), m_memoryOffset(m_memoryBuffer), m_capacity(capacity),
...
LinearAllocator::LinearAllocator(size_t capacity, size_t alignment) :
    m_parentAllocator(nullptr), m_memoryBuffer(_aligned_malloc(capacity, alignment)), m_memoryOffset(m_memoryBuffer), m_capacity(capacity),
    m_freeSpace(capacity) {
	assert(m_memoryBuffer);
	assert(capacity);
}
