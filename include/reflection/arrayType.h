...
public:
	ArrayContainer(const char* typeName, TypeId typeID, const Type* valueType, ArenaAllocator& allocator)
...
struct autoRegisterHelper<T[N]> {
	static const Type* autoRegister(Context& context) {
		...
		return context.scopedAllocator->make<ArrayContainer<ElementType, N>>(typeName, typeID, elementType, *context.arenaAllocator);
	}
};
