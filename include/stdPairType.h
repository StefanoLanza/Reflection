...
struct autoRegisterHelper<std::pair<first_type, second_type>> {
	static const Type* autoRegister(TypeDB& typeDB, Allocator& allocator) {
		...
		type->addProperty(ClassHelpers<PairType>::createFieldProperty("first", Flags::all, Semantic::none, &PairType::first, typeDB, allocator));
		type->addProperty(ClassHelpers<PairType>::createFieldProperty("second", Flags::all, Semantic::none, &PairType::second, typeDB, allocator));
